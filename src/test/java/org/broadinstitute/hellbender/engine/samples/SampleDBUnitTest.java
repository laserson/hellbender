...
public class SampleDBUnitTest extends BaseTest {
    ...
    // @TODO - lifted this from GATK since the public one is deprecated
    private final Set<Sample> getChildrenWithParents(SampleDB sampleDB, boolean triosOnly) {

        Map<String, Set<Sample>> families = sampleDB.getFamilies();
    ...
    public void testGetChildrenWithParents()
    {
        ...
        SampleDB db = builder.getFinalSampleDB();
        Assert.assertEquals(getChildrenWithParents(db, false), testKidsWithParentsFamilies2);
        Assert.assertEquals(getChildrenWithParents(db, false), testKidsWithParentsFamilies2);
        Assert.assertEquals(getChildrenWithParents(db, true), new HashSet<Sample>(Arrays.asList(new Sample("kid", "fam1", "dad", "mom", Gender.MALE,   Affection.AFFECTED))));
    }
