    ...
    public final List<Sample> parse(Reader reader, EnumSet<MissingPedField> missingFields, SampleDB sampleDB) {
        try (final XReadLines sourceReader = new XReadLines(reader, false, commentMarker)) {
            ...
            for (final String[] parts : splits) {
                ...
                if (familyPos != -1) {
                    familyID = maybeMissing(parts[familyPos]);
                }
                individualID = parts[samplePos];
                if (paternalPos != -1) {
                    paternalID = maybeMissing(parts[paternalPos]);
                }
                if (maternalPos != -1) {
                    maternalID = maybeMissing(parts[maternalPos]);
                }

                if (sexPos != -1) {
                    if (parts[sexPos].equals(SEX_MALE)) {
                        sex = Gender.MALE;
                    }
                    else if (parts[sexPos].equals(SEX_FEMALE)) {
                        sex = Gender.FEMALE;
                    }
                    else {
                        sex = Gender.UNKNOWN;
                ...
                if (phenotypePos != -1) {
                    if (isQT) {
                        ...
                        else if (parts[phenotypePos].equals(PHENOTYPE_AFFECTED)) {
            ...
            for (final Sample sample : new ArrayList<Sample>(samples)) {
                final Sample dad = maybeAddImplicitSample(sampleDB, sample.getPaternalID(), sample.getFamilyID(), Gender.MALE);
                if (dad != null) {
                    samples.add(dad);
                }

                final Sample mom = maybeAddImplicitSample(sampleDB, sample.getMaternalID(), sample.getFamilyID(), Gender.FEMALE);
                if (mom != null) {
                    samples.add(mom);
                }
            }
